...
def do_embedding(data,embedding,input_tokens,Y_tokens,nb_tags,col_id='Id'):
    # stratified columns
    stratified_col_ = 'filtered-tag-str'
    data[tools.filtered_name(Y_tokens)] = preprocessing.filter_tokens(data,Y_tokens,nb_tags)
    ...
    tmp = embed(data[input_tokens])    
    features = { 
        'X_':tmp[X_train.index],
        'Z_':tmp[X_test.index],
        'y_train_v_':y_train[tools.filtered_name(Y_tokens)].values,
        'y_test_v_':y_test[tools.filtered_name(Y_tokens)].values,
        'all_tokens_': data[tools.filtered_name(Y_tokens)].values
    }

    tools.save_emb(features,embedding)
